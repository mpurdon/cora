...
export function parseDiff(raw: string): DiffFile[] {
  ...
  let newLine = 0;
  // `--- `/`+++ ` are file headers only before a file's first `@@` hunk —
  // after that they're ordinary content lines (e.g. a deleted SQL comment
  // `-- foo` becomes `--- foo` in the diff and must not be swallowed).
  let inHunk = false;
